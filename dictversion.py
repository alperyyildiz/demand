...
class MODELL(helpful):
    ...
    def SAVE_PLOTS_V2(self,varlist):
        ...
        key2 = ''
        key3 = ''
        for dictkey in list(self.dict.keys()):
            for i,KK in enumerate(list(self.dict[dictkey])):

                if KK != 'list':
                    key = key + ' \n '
                    key2 = key2 + ' \n '
                    key = key + dictkey[0] + KK + ': '
                    key2 = key2 + dictkey[0] + KK + ' '
                    if i ==1:
                        key3 = key3 + dictkey[0] + KK  + '_'
                    else:
                        key3 = key3[:-4] + '<<>>' + dictkey[0] + KK  + '_'

                    for i,K_K in enumerate(list(self.dict[dictkey][KK].keys())):
                        print(K_K)
                        if K_K not in varlist[KK]:
                            key = key + K_K + ': ' + str(self.dict[dictkey][KK][K_K]) + ' -- '
                        else:
                            key2 = key2 + K_K + ' - '
                            key3 = key3 + K_K[0] + '-' + str(self.dict[dictkey][KK][K_K]) + '__'
...
text_file = open("k1.txt", "w")
text_file.write(k1)
text_file.close()

text_file = open("k2.txt", "w")
text_file.write(k2)
text_file.close()

text_file = open("k3.txt", "w")
text_file.write(k3)
text_file.close()
